...
import { getSellerConfig, isSellerSystemEnabled } from "@/lib/sellerConfig";

export default async function SellerGuidePage() {
  // Check seller system configuration
  const sellerConfig = await getSellerConfig();
  const sellerEnabled = isSellerSystemEnabled(sellerConfig);

  // If seller system is disabled, show disabled message
  if (!sellerEnabled) {
