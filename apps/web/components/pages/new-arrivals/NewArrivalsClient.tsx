...
const NewArrivalsPageContent = ({ categories, brands, sellers }: Props) => {
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();

  // Fixed productType for this dedicated page
  const productType = "_new_arrivals";
        ...
        <div className="mb-5">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>NewArrivals</BreadcrumbPage>
              </BreadcrumbItem>
              {category && (
                <>
                  <BreadcrumbSeparator />
                  ...
                  if (navigator.share) {
                    try {
                      await navigator.share({
                        title: "NewArrivals - " + document.title,
                        url: url,
                      });
                    } catch (err) {
...
const NewArrivalsPageClient = (props: Props) => {
  return (
    <Suspense fallback={<ShopSkeleton />}>
      <NewArrivalsPageContent {...props} />
    </Suspense>
  );
};

export default NewArrivalsPageClient;
