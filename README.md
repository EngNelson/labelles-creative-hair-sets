# Labelle's E-commerce Platform Monorepo

Welcome to the **Labelle's E-commerce Platform** – a robust, production-ready full-stack e-commerce solution built with modern technologies. Whether you are aiming to launch a complete store setup or learn enterprise-grade architecture using Turborepo, this repository has everything you need.
...
Each application module (`apps/web`, `apps/admin`, `apps/api`) has its own `.env` file where sensitive values have been removed for security.

You must fill in these values for local execution. Critical keys to locate include:

- **MongoDB**: Get `MONGO_URI` from your MongoDB Atlas cluster.
- **Firebase**: Get Web App config keys (API Key, Auth Domain, Project ID, etc.) from the Firebase Console Project Settings.
- **Stripe**: Get `STRIPE_SECRET_KEY` and Webhook secrets from the Stripe Developer Dashboard.
- **Cloudinary/ImageKit**: Get API keys from their respective dashboards for image hosting.
- Reference `docs/SETUP.md` for a comprehensive master list of all required keys.
...
**Created with ❤️ by the BlueFlex Logic Team & LabelleCH**
